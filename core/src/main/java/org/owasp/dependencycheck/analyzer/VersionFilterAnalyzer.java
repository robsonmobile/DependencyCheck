...
public class VersionFilterAnalyzer extends AbstractAnalyzer {
    ...
    protected void analyzeDependency(Dependency dependency, Engine engine) throws AnalysisException {
        ...
        if (dependency.getVersion() != null) {
            remove = dependency.getEvidence(EvidenceType.VERSION).stream().filter(e -> !dependency.getVersion().equals(e.getValue())).collect(Collectors.toSet());
        } else {
            remove = new HashSet<>();
        ...
        if (dependency.getVersion() == null) {
            Set<Evidence> evidence = dependency.getEvidence(EvidenceType.VERSION);
            final DependencyVersion version;
            Evidence e = evidence.stream().findFirst().orElse(null);
            if (e != null) {
                version = DependencyVersionUtil.parseVersion(e.getValue(), true);
                if (version != null && evidence.stream().map(ev -> DependencyVersionUtil.parseVersion(ev.getValue(), true)).allMatch(v -> version.equals(v))) {
